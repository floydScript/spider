...
class Toplist_amazon(RedisSpider):
    name = 'toplist_amazon'

    allowed_domains = ['amazon.com']

    # start_urls = ['https://www.amazon.cn/dp/B07C5H8DTP/ref=zg_bs_books_2/459-0123195-1514667?_encoding=UTF8&psc=1&refRID=NTBB32YRPRV127BGGMJ5']
    # start_urls = ['https://www.amazon.cn/dp/B076789VTJ']
    # start_urls = ['https://www.amazon.cn/dp/B07GW4RTD3']

    redis_key = 'toplist_amazon:start_urls'
    ...
    def parse_desc(self,response,tag):
        """
        封装详细信息
        :param response:
        :param tag:
        :return:
        """
        tag_list = response.xpath("//div[@id='s_contents']/div/h3/text()")
        item_list = response.xpath("//div[@id='s_contents']/div/p")
        item = ''
        for index,t in enumerate(tag_list):
            if tag in t:
                item = item_list[index].xpath("./text()")
                item = ''.join(item)
                break
        return item
